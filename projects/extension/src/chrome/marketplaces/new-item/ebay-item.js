...
chrome.runtime.onMessage.addListener(function (msg, sender, sendResponse) {
  if (msg.command == "set-item-data") {
    //NOTE: script can be injected multiple times, so we just want to make sure that it is injected once.
...
function checkDocumentState() {
  //NOTE: we're checking the document state using a different method thatn the other marketplaces, the other method didn't seem to work: onReadyStateChanged()
  let stateCheck = setInterval(() => {
    if (document.readyState === "complete") {
      swalAlert.showProcessingAlert(); //NOTE //LATER: processing alert might stop ebay from uploading images for some reason, so maybe don't show processing swal. maybe later try to fix this? Watch this: //LATER
...
async function getItemDetails(version) {
  switch (version) {
    case "one":
      fillOutEbayFormOne(
        window.itemData.properties.imageUrls,
        window.itemData.properties.title,
        window.itemData.properties.description,
        window.itemData.properties.brand,
        window.itemData.properties.condition,
        window.itemData.properties.color,
        window.itemData.properties.price,
        window.itemData.properties.sku
      );
...
async function fillOutEbayFormTwo(
  ...
  //desc-textarea //FIX: sometimes the ebay description on v2 doesn't recognize inputs and won't dispatch events, so it won't save on form unless user manually clicks it. Try to fix this, maybe I can click the bold button or something to trigger.
  ...
  //wait -> then dispatch elements for iframe to allow to process
  helpers.delay(100);

...
//   await waitForElementToLoad(".ListingCards");
//   //find card

//   console.log("listing cards found");

//   var items = document.querySelectorAll(
//     ".ListingCards .ListingCard:not(._empty)"
//   );
...
//   var imageURL = $(targetParent).find("img").attr("src");
//   var listingURL = $(targetParent).find("a.-thumbnail-link").attr("href");
...
//       //if grailed closet
//       if (window.location.href.indexOf("grailed.com/users/myitems") > -1) {
...
function getCardInfo() {
  ...
  items.forEach((item) => {
    var title = $(item).find(".listing-title").text().trim();
    var imageURL = $(item).find("img").attr("src");
    var listingURL = $(item).find("a.-thumbnail-link").attr("href");
